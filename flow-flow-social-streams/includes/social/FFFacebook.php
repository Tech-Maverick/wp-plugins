...
class FFFacebook extends FFHttpRequestFeed{
    ...
    protected function deferredInit($options, $feed) {
        ...
        if (isset($feed->{'timeline-type'})) {
            ...
            $fields    = $fields . 'id,created_time,from,message,picture,full_picture,attachments{subattachments,unshimmed_url,title,media_type,target{id},media{source}},status_type,story';
	...
	protected function getHeader($item) {        
		if (!isset($item->attachments->data[0]->media_type) || (isset($item->status_type) && $item->status_type == 'added_photos')){
			return null;
		}
    ...
    protected function showImage($item){
		...
		if (!isset($item->attachments->data[0]->media_type)){
			$this->image = $this->createImage($item->attachments->media->source);
			return true;
		}

		if ((isset($item->attachments->data[0]->target->id) && (($item->attachments->data[0]->media_type == 'photo')))){
			...
			//depricated
			$url = "https://graph.facebook.com/{$api}/{$item->attachments->data[0]->target->id}?fields=images";
			...
			if ($original_url == ''){
				...
				if (false !== $objects){
					...
					$url = "https://graph.facebook.com/{$api}/{$item->attachments->data[0]->target->id}/picture?width={$this->getImageWidth()}&type=normal";
					$original_url = $this->cache->getOriginalUrl($url);
					if ($original_url == ''){
					...
					$url = "https://graph.facebook.com/{$api}/{$item->attachments->data[0]->target->id}/picture?width=600&type=normal";
					$original_url = $this->cache->getOriginalUrl($url);
					if ($original_url == ''){
						$original_url = $this->getLocation($url);
					}
		...
		if (($item->attachments->data[0]->media_type == 'video')
			...
			if (!isset($item->attachments->data[0]->target->id) && isset($item->attachments->data[0]->unshimmed_url) && strpos($item->attachments->data[0]->unshimmed_url, 'facebook.com') > 0 && strpos($item->attachments->data[0]->unshimmed_url, '/videos/') > 0){
			 	$path = parse_url($item->attachments->data[0]->unshimmed_url, PHP_URL_PATH);
				$tokens = explode('/', $path);
				if (empty($tokens[sizeof($tokens)-1])) unset($tokens[sizeof($tokens)-1]);
				$item->attachments->data[0]->target->id = $tokens[sizeof($tokens)-1];
			}
			if (isset($item->attachments->data[0]->target->id) && trim($item->attachments->data[0]->target->id) != ''){
				if (isset($item->attachments) && isset($item->attachments->data) && sizeof($item->attachments->data) > 0){
					$subattachments = $this->getSubattachments($item);
					if (sizeof($subattachments) > 0){
						$image= $subattachments[0];
						...
						$this->media = $this->createMedia('http://www.facebook.com/video/embed?video_id=' . $item->attachments->data[0]->target->id, $width, $height, 'video');
						return true;
					}
				}
				//deprecated
				$data = $this->getFeedData($this->getUrlWithToken( "https://graph.facebook.com/{$api}/{$item->attachments->data[0]->target->id}?fields=embed_html,source" ));
			...
			else if (isset($item->attachments->data[0]->media->source)){
				if (strpos($item->attachments->data[0]->media->source, 'giphy.com') > 0) {
					$arr = parse_url( urldecode( $item->attachments->data[0]->media->source ) );
				...
				if (isset($item->full_picture)){
					...
					$type = (strpos($item->attachments->data[0]->media->source, '.mp4') > 0) ? 'video/mp4' : 'video';
					$this->media = $this->createMedia($item->attachments->data[0]->media->source, 600, FFFeedUtils::getScaleHeight(600, $this->image['width'], $this->image['height']), $type);
					return true;
				}
		...
		if ((($item->attachments->data[0]->media_type == 'link') || ($item->attachments->data[0]->media_type == 'event')) && isset($item->picture)){
			...
			if (isset($item->full_picture)){
				$this->image = $this->createImage($item->full_picture);
				$this->media = $this->createMedia((pathinfo($item->attachments->data[0]->unshimmed_url, PATHINFO_EXTENSION) === 'gif') ? $item->attachments->data[0]->unshimmed_url : $item->full_picture, null, null, 'image', true);
				return true;
			}
		...
		if ($item->attachments->data[0]->media_type == 'status' && isset($item->attachments->data) && sizeof($item->attachments->data) > 0){
			$subattachments = $this->getSubattachments($item);
			if (sizeof($subattachments) > 0){
				$image = $subattachments[0];
				$this->image = $this->createImage($image->src, $image->width, $image->height);
    ...
    protected function getContent($item){
	    if (!isset($item->attachments->data[0]->media_type) && isset($item->attachments->data[0]->title)) return (string)$item->attachments->data[0]->title;
    ...
    protected function getPermalink( $item ) {
	    if (!isset($item->attachments->data[0]->media_type)){
		    return $item->attachments->data[0]->unshimmed_url;
	    }
	...
	private function getSubattachments($item){
		...
		if (isset($item->attachments) && isset($item->attachments->data) && sizeof($item->attachments->data) > 0){
			...
			if (isset($data->media->image)){
				if ($item->attachments->media_type == 'link' && isset($item->status_type) && $item->status_type == 'shared_story'){}
